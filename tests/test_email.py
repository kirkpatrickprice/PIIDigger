...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {"email": {"joh********4@company-name.co.uk"}},
        ),
        (
...
class TestEmailFunctions:
    def test_is_valid(self):
        ...
        assert (
            email._is_valid("user@thisisasuperlongdomainlabelofexactlysixtysevencharactersinlengthxxx.com") is False
        )  # Invalid domain label length
        assert email._is_valid("user@invalid.0tld") is False  # Invalid TLD
        assert (
            email._is_valid(
                "user@example.thisisaverylongdomainnamethatexceedsthemaximumallowedlengthforadomainpartof253charactersandshouldbeusedtotestemailvalidationroutines.com"
            )
            is False
